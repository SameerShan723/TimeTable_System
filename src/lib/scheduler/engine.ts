...
function sortUnitsByConstraint(units: ClassUnit[]): ClassUnit[] {
  ...
  return [...units].sort((a, b) => {
    ...
    return (demandBySubject.get(b.subject) || 0) - (demandBySubject.get(a.subject) || 0);
  });
}
...
export function generateDeterministicTimetable(options: SchedulerOptions): ScheduleResult {
  ...
  const units = sortUnitsByConstraint(rawUnits);

  const unscheduled: ClassUnit[] = [];

  // Helper to choose target day order to balance teacher/section loads
  function preferredDaysFor(unit: ClassUnit): string[] {
    // Sort days by combined (teacher + section) count ascending to promote spread
    return [...Days].sort((d1, d2) => {
      ...
      return Days.indexOf(d1) - Days.indexOf(d2);
    });
  }
  ...
  for (const unit of units) {
    ...
    for (const day of [...daysNoSubjYet, ...daysAllowRepeat]) {
      ...
      const times = timePreferenceBase;
      ...
      for (const time of times) {
        ...
        // Prefer rooms with fewer existing classes that day (to spread usage)
        const sortedRooms = [...roomsToTry].sort((r1, r2) => {
          ...
          return roomsToTry.indexOf(r1) - roomsToTry.indexOf(r2);
        });
